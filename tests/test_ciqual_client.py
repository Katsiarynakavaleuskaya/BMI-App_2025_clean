    ...
    with tempfile.NamedTemporaryFile(
        ...
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "Zinc", "Potassium", "Sélénium", "Iode",
                "Vitamine A", "Vitamine D", "Vitamine C",
...
def test_ciqual_client_parse_food_item_with_invalid_values():
    ...
    # Should not be included
    assert "fat_g" not in food_item.nutrients_per_100g
    # Should not be included
    assert "carbs_g" not in food_item.nutrients_per_100g
    # Should be included
    assert food_item.nutrients_per_100g["fiber_g"] == 2.4


def test_ciqual_client_parse_food_item_value_error():
    """Test parsing with ValueError in nutrient parsing."""
    client = CIQUALClient()
    ...
    food_data = {
        ...
        "alim_nom_eng": "Apple, raw",
        "alim_grp_nom_fr": "Fruits",
        # Will cause ValueError when converting to float
        "Protéines": "invalid",
    }
...
def test_ciqual_client_parse_food_item_error_handling():
    ...
    with patch(
        'core.food_apis.ciqual_client.CIQUALFoodItem.__init__',
...
def test_ciqual_client_error_handling():
    ...
    with patch(
        'core.food_apis.ciqual_client.open',
    ...
    with patch(
        'core.food_apis.ciqual_client.CIQUALClient._parse_food_item',
...
def test_ciqual_client_load_database_file_not_found():
    ...
    with patch('core.food_apis.ciqual_client.logger.warning') as mock_logger:
        CIQUALClient(data_file="non_existent_file.csv")
...
def test_ciqual_client_load_database_file_error():
    ...
    try:
        ...
        with patch('core.food_apis.ciqual_client.logger.error') as mock_logger:
            CIQUALClient(data_file=temp_path)
            # Should log an error
            mock_logger.assert_called()
    finally:
...
def test_ciqual_client_search_foods_exception():
    ...
    with patch('core.food_apis.ciqual_client.logger.error') as mock_logger:
        with patch.object(
            client, '_parse_food_item', side_effect=Exception("General error")
        ):
            results = client.search_foods("test")
...
def test_ciqual_client_get_all_food_groups_exception():
    ...
    with patch('core.food_apis.ciqual_client.logger.error') as mock_logger:
        with patch(
            'core.food_apis.ciqual_client.sorted',
            side_effect=Exception("General error"),
        ):
            groups = client.get_all_food_groups()
            # Should return empty list on error
