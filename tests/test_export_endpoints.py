...
class TestExportEndpoints:
    ...
    def test_export_daily_csv_success(self):
        ...
        response = self.client.get("/api/v1/premium/exports/day/test_plan.csv", headers={"X-API-Key": "test_key"})
    ...
    def test_export_weekly_csv_success(self):
        ...
        response = self.client.get("/api/v1/premium/exports/week/test_plan.csv", headers={"X-API-Key": "test_key"})
        assert response.status_code == 200
        assert response.headers["content-type"] == "text/csv; charset=utf-8"
        assert "attachment" in response.headers["content-disposition"]
    ...
    def test_export_daily_pdf_success(self):
        ...
        response = self.client.get("/api/v1/premium/exports/day/test_plan.pdf", headers={"X-API-Key": "test_key"})
    ...
    def test_export_weekly_pdf_success(self):
        ...
        response = self.client.get("/api/v1/premium/exports/week/test_plan.pdf", headers={"X-API-Key": "test_key"})
        # PDF export might fail if ReportLab is not installed, which is expected in test environment
        assert response.status_code in [200, 503]
